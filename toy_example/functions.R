...
parameter_tuning_rf <- function(y, X, Dhour, days, left_out_days = 7, seed = 1){
  ...
  param_space <- makeParamSet(
    makeIntegerParam("mtry", lower = 2, upper = 10), #change to 7 for example #50
...
fit_base_forecast_oos <- function(x, base_method = "naive", ts_aggr_orders, seasonal_frequency = NULL,
  ...
  if(base_method == "naive"){
    ...
    for(ts in 1:n_ts){
      miter <- ts_aggr_orders[ts]
      fit_naive <- demand_window_insample[[ts]][-c((nrow(demand_window_insample[[ts]]) - Whour/miter + 1) : nrow(demand_window_insample[[ts]])),] #-c(oos_indices_fix[[ts]] - (Whour/miter))
...
fit_ml_reconciliation <- function(inputlist_ML_base_forecasts, n_cs, n_cs_up, n_cs_bt, C, ts_aggr_orders, delivery_areas, 
  ...
  for(csl in 1:n_cs_bt){
    ...
    if(is.null(paramslist)){
      ...
      params = as.list(paramslist[[csl]])
    }
    
    set.seed(seed_mat[csl])
    # Train ML
    if(ML_method == "randomforest"){
      fit_RF_own <- fit_rf(y = y, X = Xin, Xout = Xout, parlist = params)
      recon_bt_forecasts[,csl] <- pmax(0,round(fit_RF_own$forecasts))
    }
    if(ML_method == "xgboost"){
      if(ML_loss == "squared"){
        fit_xgboost_own <- fit_xgboost(y = y, X = Xin, Xout = Xout, parlist = params)
        recon_bt_forecasts[,csl] <- pmax(0,round(fit_xgboost_own$forecasts))
      }
      if(ML_loss == "tweedie"){
        fit_xgboost_own <- fit_xgboost_tweedie(y = y, X = Xin, Xout = Xout, parlist = params, tweedie_variance_power = tweedie_variance_power)
        recon_bt_forecasts[,csl] <- pmax(0,round(fit_xgboost_own$forecasts))
      }
    }
    if(ML_method == "lightgbm"){
      if(ML_loss == "squared"){
        fit_lightgbm_own <- fit_lightgbm(y = y, X = Xin, Xout = Xout, parlist = params)
        recon_bt_forecasts[,csl] <- pmax(0,round(fit_lightgbm_own$forecasts))
      }
      if(ML_loss == "tweedie"){
        fit_lightgbm_own <- fit_lightgbm_tweedie(y = y, X = Xin, Xout = Xout, parlist = params, tweedie_variance_power = tweedie_variance_power)
        recon_bt_forecasts[,csl] <- pmax(0,round(fit_lightgbm_own$forecasts))
      }
    }
