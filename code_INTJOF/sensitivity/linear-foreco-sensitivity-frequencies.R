# Linear forecast reconcilation script with subset of the frequencies
...
### FoReco parameters ###
# FoReco_method = "oct" # tcs, cst, ite, oct, ctbu
...
ts_aggr_orders_old = c(1,2,3,4,6,8,12,16,24,48) #30-min, 1-hour, 1.5-hour, 2-hour, 3-hour, 4-hour, 6-hour, 8-hour, 12-hour, 24-hour
...
if(base_method == "averageBF"){
  idx_help_ts_old = cumsum(rev((window_hours + validation - Whour)/ts_aggr_orders_old))
  idx_help_ts_sel = c(idx_help_ts_old[1]+1, idx_help_ts_old[8])
}else{
  idx_help_ts_old = cumsum(rev((window_hours + validation)/ts_aggr_orders_old))
  idx_help_ts_sel = c(idx_help_ts_old[1]+1, idx_help_ts_old[8])
}
...
FoReco_method_name = paste0(FoReco_method_name, "_subfreq")
...
for(iwindow in rolling_sequence){
  ...
  basef_all = inputlist_FoReco_package[[idx]]$base
  resids_all = inputlist_FoReco_package[[idx]]$residuals
  # select only the frequencies daily, hourly, 30min 
  basef = basef_all[,-c(2:52)]
  resids = resids_all[, -c(idx_help_ts_sel[1]:idx_help_ts_sel[2])]
