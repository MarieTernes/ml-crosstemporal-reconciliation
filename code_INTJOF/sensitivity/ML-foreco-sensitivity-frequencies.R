# ML reconcilation script with subset of the frequencies
...
ts_aggr_orders = c(1,2,48) #30-min, 1-hour, 1.5-hour, 2-hour, 3-hour, 4-hour, 6-hour, 8-hour, 12-hour, 24-hour
...
ML_name = paste0(ML_name, "_subfreq")
...
dir.create(file.path(".","subfreq"), recursive = TRUE, showWarnings = FALSE)
setwd(paste0("./", "subfreq"))

ptm_total = proc.time()
for(iwindow in rolling_sequence){
  ptm = proc.time()
  # print(paste("rolling window",which(iwindow==rolling_sequence), "of", length(rolling_sequence)))
  #### Prepare Data ####
  ...
  # select only the frequencies 30min, hourly, daily 
  inputlist_ML_base_forecasts_subset_freq <- vector(mode = "list")
  inputlist_ML_base_forecasts_subset_freq$Y <- inputlist_ML_base_forecasts[[idx]]$Y
  inputlist_ML_base_forecasts_subset_freq$Xin <- inputlist_ML_base_forecasts[[idx]]$Xin[,-c((n_cs*2+1):(n_cs*9))]
  inputlist_ML_base_forecasts_subset_freq$Xout <- inputlist_ML_base_forecasts[[idx]]$Xout[,-c((n_cs*2+1):(n_cs*9))]
  ...
  ml_reconcile <- fit_ml_reconciliation(inputlist_ML_base_forecasts = inputlist_ML_base_forecasts_subset_freq, n_cs = n_cs, n_cs_up = n_cs_up, n_cs_bt = n_cs_bt, C = C, 
  ...
  for(ts in 1:n_ts){
    ...
    for(csl in 1:n_cs){
      ...
      }else{ #this holds now for the weekly forecast, however probably not generalizable for all forecasts with Dhour/miter < 1
