# ML forecast reconcilation script with large feature matrix (entire crosstemporal tree)
...
# ML_method = "bottomup" #options: bottomup, xgboost, lightgbm, randomforest 
ML_loss = "squared" #options: squared, tweedie (only for xgboost and lightgbm)
ML_predictor = "cstempall" #cstemp #cs #temp #cstempall (predictors of the machine learning model: cross-temporal, cross-sectional, temporal, all)
...
dir.create(file.path(".","cstempall"), recursive = TRUE, showWarnings = FALSE)
setwd(paste0("./", "cstempall"))
